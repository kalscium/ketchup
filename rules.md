...
- ## Precedence Jumptable Array
	...
	- When inserting a binary or unary right-aligned node, first iterate through the jumptable starting with the precedence of the node itself, and if it finds a node of a higher precedence (`Some(idx)`) then insert the node to that index and then update it's precedence in the jumptable and then increment all the jumptable entries of higher precedence by one
	- If it can't find a entry of higher precedence in the jumptable (for equal precedence refer to node recursion), then simply insert to the end of the ASA (not push) (replace the last node)
- ## Node recursion
